...
/** Section 7 – Implementation This file contains all code examples from section
  * 7 of our paper.
  *
  ...
  * addition to repeating the few examples from the paper in this file we
  * explain the relation between the implementation in the paper and in the
  * artifact.
  *
...
trait Section7 { self: RichParsers =>
  ...
  /** Section 7.1, introduces the concrete type of a parser as
    *
    * trait P[+R] { def results: Res[R] def derive: Elem => P[R] }
    *
    ...
    * implementation of the interface defined in `Parsers.scala` (corresponding
    * to Figure 1a.).
    *
    ...
    *     {...}`) the various combinators are implemented by named classes /
    *     objects (that is, `object Fail extends P[Nothing] { ... }`).
